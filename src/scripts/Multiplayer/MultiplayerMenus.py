...
def CreateBoolButton(pName, eType, pDestination, bState = 0, fWidth = 0.0, fHeight = 0.0):
	debug(__name__ + ", CreateBoolButton")
...
def CreateRoundedTextEntry(pDefault, pcConfigName, kColor, fWidth, fHeight, iMaxChars):
	debug(__name__ + ", CreateRoundedTextEntry")
...
def BuildMultiplayerMenus():
	...
	debug(__name__ + ", BuildMultiplayerMenus")
	pTopWindow = App.TopWindow_GetTopWindow()
...
def BuildMultiplayerPreGameMenus(bForceDoItAgain = 1):
	debug(__name__ + ", BuildMultiplayerPreGameMenus")
...
def BuildTopButtons(pPane, pDatabase):
	debug(__name__ + ", BuildTopButtons")
	global g_pMultiplayerPane
...
def CreateSortButton(pName, pDestination, pSortBy, bStringField, fWidth = 0.0, fHeight = 0.0):
	debug(__name__ + ", CreateSortButton")
...
def BuildServerMenu(pPane, pDatabase):
	# Get LCARS string.
	debug(__name__ + ", BuildServerMenu")
	pcLCARS = App.GraphicsModeInfo_GetCurrentMode().GetLcarsString()
...
def BuildClientPane(pPane, pDatabase):
	debug(__name__ + ", BuildClientPane")
...
def getGameName(GameName):
        debug(__name__ + ", getGameName")
        import string
        GameNameList = string.split(GameName, "|")

        try:
                import gameinfo
                GAME_TAG = gameinfo.GAME_TAG
        except:
                GAME_TAG = ""
        
        # remove gametag from list
        if GameNameList[0] == GAME_TAG + " ":
                GameNameList  = GameNameList[1:]
        # remove leading space from list
        if GameNameList[0][0] == " ":
                GameNameList = GameNameList[0][1:]
        
        GameName = ""
        for s in GameNameList:
                GameName = GameName + s
...
#	g_pHostPlayerNameInput = CreateRoundedTextEntry(pDefault, "Player Name", App.g_kTextEntryBackgroundColor,
	...
	if (App.g_kConfigMapping.HasValue("Multiplayer Options", "Game Name")):
        ...
        # by Defiant
        pDefault = App.TGString(getGameName(pDefault.GetCString()))
...
def BuildEyeCandyPane(pDatabase):
	debug(__name__ + ", BuildEyeCandyPane")
	kSizes =  [6, 9, 9, 12, 15]
...
def BuildStatusWindow(pMultWindow, pDatabase):
	debug(__name__ + ", BuildStatusWindow")
...
def BuildChatWindow (pMultWindow, pDatabase):
	debug(__name__ + ", BuildChatWindow")
...
def HandleCancel (TGObject, pEvent):
	# Bring up options again.
	# Find the multiplayer window
	debug(__name__ + ", HandleCancel")
	pTopWindow = App.TopWindow_GetTopWindow()
	pMultWindow = App.MultiplayerWindow_Cast (pTopWindow.FindMainWindow(App.MWT_MULTIPLAYER))
...
def HandleCancelConnect (TGObject, pEvent):
	...
	debug(__name__ + ", HandleCancelConnect")
...
def HandleServerEntry (TGObject, pEvent):
	...
	debug(__name__ + ", HandleServerEntry")
...
def HandleServerPlayer (TGObject, pEvent):
	...
	# Set the font small
	debug(__name__ + ", HandleServerPlayer")
	App.g_kFontManager.SetDefaultFont(MainMenu.mainmenu.g_pcFlightSmallFont,
				MainMenu.mainmenu.g_kFlightSmallFontSize[MainMenu.mainmenu.g_iRes])
...
def HandleRefreshServerListDone (TGObject, pEvent):
	...
	debug(__name__ + ", HandleRefreshServerListDone")
...
def KeyboardHandler(pObject, pEvent):
	debug(__name__ + ", KeyboardHandler")
...
def SendChat ():
	...
	kStream.OpenBuffer (MissionShared.NET_BUFFER_SIZE)			# Open the buffer stream with a 256 byte buffer.
	
	# Write relevant data to the stream.
	# First write message type.
	if (pMultWindow.IsTeamChat ()):
		pcString = "(Team) " + pcString
			...
			if (pNetwork):
				...
				if (pNetPlayer):
					...
					# Create string to display based on name and the string from the buffer
                                        # Safe into Saffis log
                                        App.STMissionLog_GetMissionLog().AddLine(App.TGString(pcFullString))
                                        # Safe into file:
                                        import nt
                                        file = nt.open("scripts/Custom/chatlog.txt", nt.O_WRONLY | nt.O_APPEND | nt.O_CREAT)
                                        if file:
                                                nt.write(file, pcFullString + "\n")
                                                nt.close(file)
                                        else:
                                                print "unable to write to file scripts/Custom/chatlog.txt"
					...
					if MissionShared.g_bGameOver:
						...
						pSubtitleAction.SetDuration (10.0)
...
def UnloadPreGameMenus():
	...
	debug(__name__ + ", UnloadPreGameMenus")
...
def HandleServerListState(pObject, pEvent):
	...
	# it
	debug(__name__ + ", HandleServerListState")
...
def HandleServerListPercentage(pObject, pEvent):
	debug(__name__ + ", HandleServerListPercentage")
...
def RoundedTextEntryMouseHandler (pSelf, pEvent):
	debug(__name__ + ", RoundedTextEntryMouseHandler")
...
def ExitMultiplayer(pObject, pEvent):
	debug(__name__ + ", ExitMultiplayer")
...
def ReturnToMainMenu(pObject, pEvent):
	...
	# Make sure we're don't have any GameSpy queries running
	debug(__name__ + ", ReturnToMainMenu")
	pEvent = App.TGBoolEvent_Create()
	pEvent.SetEventType(App.ET_REFRESH_SERVER_LIST)
	pEvent.SetDestination(g_pMultiplayerPane)
	pEvent.SetBool(1)
...
def UpdateConfigManager():
	...
	debug(__name__ + ", UpdateConfigManager")
	...
	App.TGParagraph_Cast(App.TGPane_Cast (g_pHostGameNameInput.GetNthChild(0)).GetNthChild(0)).GetString(pGameName)
        ...
        # change by Defiant: add Game Version Tag before the server name
        try:
                import gameinfo
                GAME_TAG = gameinfo.GAME_TAG
        except:
                GAME_TAG = ""
        pGameName = App.TGString(GAME_TAG + " | " + pGameName.GetCString())
...
def HandleDedicatedClicked(pObject, pEvent):
	debug(__name__ + ", HandleDedicatedClicked")
...
def ToggleDirectJoin(pObject, pEvent):
	...
	debug(__name__ + ", ToggleDirectJoin")
	if g_pClientDirectButton.IsChosen():
...
def ToggleLocalInternetHost(pObject, pEvent):
	...
	debug(__name__ + ", ToggleLocalInternetHost")
...
def HandleSetMissionName(TGObject, pEvent):
	...
	debug(__name__ + ", HandleSetMissionName")
...
def HandleClientStartClicked(TGObject, pEvent):
	# We have to update the configuration manager with the latest data
	debug(__name__ + ", HandleClientStartClicked")
	UpdateConfigManager()
...
def HandleHostStartClicked(TGObject, pEvent):
	# We have to update the configuration manager with the latest data
	debug(__name__ + ", HandleHostStartClicked")
	UpdateConfigManager()
...
def HandleStartGame(pMultWindow, pEvent):
	...
	debug(__name__ + ", HandleStartGame")
	...
	import Custom.Autoload.AddOnShip
	Custom.Autoload.AddOnShip.CheckFolders()
...
###############################################################################
def HideMissionPane():
	debug(__name__ + ", HideMissionPane")
