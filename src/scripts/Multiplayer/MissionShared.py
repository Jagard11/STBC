...
def Terminate(pMission):
	...
	MissionMenusShared.g_iPlayerLimit = MissionMenusShared.NORMAL_PLAYER_NUM
...
def CreateTimeLeftTimer (iTimeLeft):
	debug(__name__ + ", CreateTimeLeftTimer")
	import MissionLib
...
def UpdateTimeLeftHandler (pObject, pEvent):
	debug(__name__ + ", UpdateTimeLeftHandler")
	import MissionMenusShared
	if (g_bGameOver):
...
def EndGame(iReason = END_ITS_JUST_OVER):
	...
	kStream.OpenBuffer (NET_BUFFER_SIZE)				# Open the buffer stream with a 256 byte buffer.
...
def CreateSubtitledSoundAction (pDatabase, pcSoundName):
	debug(__name__ + ", CreateSubtitledSoundAction")
	import MissionLib
...
def SoundDoneHandler (pObject, pEvent):
	debug(__name__ + ", SoundDoneHandler")
