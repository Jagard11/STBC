...
def GetWinString():
	...
	import Mission2Menus
	pSortList.sort(Mission2Menus.CompareTeams)
	...
	for iTeamId in pSortList:
		if g_kTeamScoreDictionary.has_key(iTeamId):
			...
			iScore = 0
		if iScore == iTopScore:
			pString = pDatabase.GetString("Team")
			pcString = pString.GetCString() + " " + str(iTeamId + 1)
			pTopTeams.append(pcString)
	...
	elif len(pTopTeamNums) > 0 and Mission2Menus.g_iTeam == pTopTeamNums[0]:
		DynamicMusic.PlayFanfare("Win")
	else:
...
#	debug("Terminating multiplayer mission 1.")
	...
	Mission2Menus.g_fYPixelOffset = 0.0
	Mission2Menus.g_fXPixelOffset = 0.0

	Mission2Menus.g_iTeam = 0
	Mission2Menus.g_iIdOfCurrentlySelectedPlayer = App.TGNetwork.TGNETWORK_INVALID_ID

	# Global pointers to user interface items
	Mission2Menus.g_pTeamButton = None
	Mission2Menus.g_pOptionsWindowBootButton = None
	Mission2Menus.g_pOptionsWindowPlayerMenu = None
...
#	debug("Multiplayer mission start.")
	...
	if (App.g_kUtopiaModule.IsHost ()):
		Mission2Menus.BuildMission2Menus ()
	...
	# Now we're done.  The menu will do the work to create the ship.
        ...
        # watch ships we have to rescale:
        import Custom.QBautostart.Rescale
        Custom.QBautostart.Rescale.init()


def TorpedoFiredPlaySound(pObject, pEvent):
	debug(__name__ + ", TorpedoFiredPlaySound")
...
#			debug("Received " + str (iScoreCount) + "scores")
			...
			Mission2Menus.RebuildPlayerList ()
...
def InitNetwork (iToID):
	...
	kStream.OpenBuffer (Multiplayer.MissionShared.NET_BUFFER_SIZE)				# Open the buffer stream with a 256 byte buffer.
...
#		iKilledPlayerID = 0
		# Not a ship.  Don't award score and kills
		return
	...
	if (pNetwork):
		...
		if (iFiringPlayerID != 0):
			...
			if (pPlayer):
				...
				if (g_kTeamDictionary.has_key (iFiringPlayerID)):
					iTeam = g_kTeamDictionary [iFiringPlayerID]
					pcFiringTeamName = pDatabase.GetString ("Team" + str (iTeam + 1) + " Name")
		...
		if (iKilledPlayerID != 0):
			...
			if (g_kTeamDictionary.has_key (iKilledPlayerID)):
				...
				pcKilledTeamName = pDatabase.GetString ("Team" + str (iTeam + 1) + " Name")
...
def ResetEnemyFriendlyGroups ():
	...
	iOurTeam = Mission2Menus.g_iTeam
