...
#	Return:	none
###############################################################################
def BuildMission2Menus(bRebuild = 0):
	debug(__name__ + ", BuildMission2Menus")
...
def BuildMissionMenu (pMultWindow, pDatabase):
	...
	pMissionPane = Multiplayer.MissionMenusShared.BuildMissionMenu(pMultWindow, pDatabase,
		"Team Death-Match Host", "Team Death-Match Client", "Team Death-Match Direct Host")
	...
	g_pTeamButton = App.STRoundedButton_CreateW(pDatabase.GetString ("Team" + str (g_iTeam + 1) + " Name"),
					 pEvent, TEAM_TOGGLE_WIDTH, TEAM_TOGGLE_HEIGHT, 1)
...
def RebuildPlayerList():
	...
	pSortList = []
	pTeamScoresList = []
	pTeamScoresDict = Mission2.g_kTeamScoreDictionary
	pTeamKillsDict = Mission2.g_kTeamKillsDictionary
...
#		print ("Rebuilding player list.got menu")
		...
		pDict = Mission2.g_kKillsDictionary
		...
		pTeamDict = Mission2.g_kTeamDictionary 
		...
		for iTeam in pTeamScoresList:
			...
			iCount = 0
			for pPlayer in pSortList:
				iPlayerTeam = Mission2.INVALID_TEAM
				if (pTeamDict.has_key (pPlayer.GetNetID ())):
					iPlayerTeam = pTeamDict [pPlayer.GetNetID ()]
...
def CreateScoreboardPlayerScoreEntry(pPlayer):
	...
	pDict = Mission2.g_kScoresDictionary
	...
	# Create death total
	# Get the death total from the score dictionaries
	pDict = Mission2.g_kDeathsDictionary
...
def CreateTeamAceEntry (pPlayer):
	...
	# Get the kill total from the score dictionaries
	pDict = Multiplayer.Episode.Mission2.Mission2.g_kKillsDictionary
	...
	# Create the score
	# Get the death total from the score dictionaries
	pDict = Multiplayer.Episode.Mission2.Mission2.g_kScoresDictionary
...
def ComparePlayer (pThisPlayer, pOtherPlayer):
	...
	# Get the score total from the score dictionaries
	pDict = Mission2.g_kScoresDictionary
	...
	if (iThisScore < iOtherScore):
		...
		pKillsDict = Mission2.g_kKillsDictionary
...
def CompareTeams (iThisTeam, iOtherTeam):
	...
	pDict = Mission2.g_kTeamScoreDictionary
...
#	App.TopWindow_GetTopWindow().AllowShowOptionsWindow(1)
	...
	if (App.g_kUtopiaModule.IsClient ()):
		...
		if (pNetwork):
			...
			kStream.WriteChar (chr (Mission2.TEAM_MESSAGE))
	...
	if (App.g_kUtopiaModule.IsHost () and (not App.g_kUtopiaModule.IsClient ())):
		...
		Mission2.g_kTeamDictionary [pNetwork.GetLocalID ()] = g_iTeam
...
def SelectTeamHandler (pSelf, pEvent):
	...
	# Update interface.
	g_pTeamButton.SetName (pDatabase.GetString ("Team" + str (g_iTeam + 1) + " Name"))
...
def BuildMissionSpecificOptionsMenuPane(pEventPane):
	...
	if App.g_kUtopiaModule.IsHost() and App.g_kUtopiaModule.IsClient():
		pSubtitle = Multiplayer.MissionShared.g_pDatabase.GetString("Team Death-Match Host")
	elif App.g_kUtopiaModule.IsClient():
		pSubtitle = Multiplayer.MissionShared.g_pDatabase.GetString("Team Death-Match Client")
	else:
		pSubtitle = Multiplayer.MissionShared.g_pDatabase.GetString("Team Death-Match Direct Host")
