...
#	Return:	none
###############################################################################
def BuildMission5Menus(bRebuild = 0):
	debug(__name__ + ", BuildMission5Menus")
...
#	Multiplayer.MissionMenusShared.g_pFragLimitButton.SetDisabled(0)
	...
	g_pTeamButton = App.STRoundedButton_CreateW(pDatabase.GetString("Attackers"), pEvent, TEAM_TOGGLE_WIDTH, TEAM_TOGGLE_HEIGHT, 1)
...
def RebuildPlayerList():
	...
	pTeamScoresDict = Mission5.g_kTeamScoreDictionary
	pTeamKillsDict = Mission5.g_kTeamKillsDictionary
...
#		print ("Rebuilding player list.got menu")
		...
		pDict = Mission5.g_kKillsDictionary
		...
		pTeamDict = Mission5.g_kTeamDictionary 
		...
		for iTeam in pTeamScoresList:
			...
			iCount = 0
			for pPlayer in pSortList:
				iPlayerTeam = Mission5.INVALID_TEAM
				if (pTeamDict.has_key (pPlayer.GetNetID ())):
					iPlayerTeam = pTeamDict [pPlayer.GetNetID ()]
...
def CreateScoreboardPlayerScoreEntry(pPlayer):
	...
	pDict = Mission5.g_kDeathsDictionary
...
def CreateTeamAceEntry (pPlayer):
	...
	# Get the kill total from the score dictionaries
	pDict = Multiplayer.Episode.Mission5.Mission5.g_kKillsDictionary
	...
	# Get the death total from the score dictionaries
	pDict = Multiplayer.Episode.Mission5.Mission5.g_kDeathsDictionary
...
def ComparePlayer (pThisPlayer, pOtherPlayer):
	...
	# Get the kill total from the score dictionaries
	pDict = Mission5.g_kScoresDictionary
	...
	if (iThisScore < iOtherScore):
		...
		pKillsDict = Mission5.g_kKillsDictionary
...
def CompareTeams (iThisTeam, iOtherTeam):
	...
	pDict = Mission5.g_kTeamScoreDictionary
...
#	App.TopWindow_GetTopWindow().AllowShowOptionsWindow(1)
	...
	if (App.g_kUtopiaModule.IsClient ()):
		...
		if (pNetwork):
			...
			kStream.WriteChar (chr (Mission5.TEAM_MESSAGE))
	...
	if (App.g_kUtopiaModule.IsHost () and (not App.g_kUtopiaModule.IsClient ())):
		...
		Mission5.g_kTeamDictionary [pNetwork.GetLocalID ()] = g_iTeam
