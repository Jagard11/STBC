...
def Terminate(pMission):
	...
	Mission3Menus.g_pOptionsWindowPlayerMenu = None
        ...
        if Mission3Menus.qbGameMode:
		if Mission3Menus.QBGameModeActivated:
                	Mission3Menus.qbGameMode.Deactivate()
                Mission3Menus.qbGameMode = None
        Mission3Menus.StartMissionRunOnce = 0
	Mission3Menus.QBGameModeActivated = 0
	Mission3Menus.BridgeStartOnce = 0
	Mission3Menus.Mission3StartOnce = 0
	Mission3Menus.g_iSystem = []
	Mission3Menus.lastSystem = 0
...
def Initialize(pMission):
        ...
        Mission3Menus.LoadQBautostart(0)
...
#			debug("Creating system")
			...
			Mission3Menus.lastSystem = kStream.ReadInt()
			Multiplayer.MissionShared.g_pStartingSet = Multiplayer.SpeciesToSystem.CreateSystemFromSpecies(Mission3Menus.lastSystem)
			Mission3Menus.g_iSystem = []
                        while(1):
                                mySystem = kStream.ReadInt()
                                if mySystem == 0:
                                        break
				if mySystem != Mission3Menus.lastSystem:
					Mission3Menus.g_iSystem.append(mySystem)
...
def InitNetwork(iToID):
	...
	kStream.WriteInt(Mission3Menus.lastSystem)
	
	for mySystem in Mission3Menus.g_iSystem:
...
def AddShipNameToGroup(pShip):
	...
	dAllTeams = Mission3Menus.g_dAllTeams
        pEnemyGroup = MissionLib.GetEnemyGroup()
        pFriendlyGroup = MissionLib.GetFriendlyGroup()
...
def ObjectCreatedHandler(pObject, pEvent):
	...
	if pShip:
		...
		#if Mission3Meus.CheckActiveMutator("NanoFX v2.0 BETA"):
...
def RestartGameHandler(pObject, pEvent):
	...
	pNetwork.SendTGMessage(0, pMessage)
        ...
        # Load new Bridge
        #Mission3Menus.LoadBridge()
        
        # Restart Engineering
        Mission3Menus.LoadQBautostart(2)
...
def ResetEnemyFriendlyGroups():
	...
	if (pNetwork and pGame):
		...
		for sRace in Mission3Menus.g_dAllTeams.keys():
			iTeam = Mission3Menus.g_dAllTeams[sRace]
