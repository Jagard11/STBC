...
def BuildMission3Menus(bRebuild = 0):
	debug(__name__ + ", BuildMission3Menus")
...
def TargetChanged(pObject, pEvent):
	...
	Mission3.SendTargetChanged(pPlayer)
...
def StartMission(iSpecies, iSystem):
	...
	global Mission3StartOnce
	if not Mission3StartOnce and not useBridge:
	...
	#if App.g_kUtopiaModule.IsHost():
		# Create the Ships
		#Mission3.CreateShips()
	
	if not Mission3StartOnce:
		Mission3StartOnce = 1
...
def RebuildShipSelectWindow():
	...
	for iIndex in range(1, Multiplayer.SpeciesToShip.MAX_FLYABLE_SHIPS):
		...
		if Races[sSelectedTeam].IsOurRace(sShipScript):
			# Setup the event for when this button is clicked
			pEvent = App.TGIntEvent_Create()
