...
def Initialize():
	# Create the set ("Nursery1")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Nursery1")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Nursery.Nursery1")

	# Activate the proximity manager for our set.
	pSet.SetProximityManagerActive(1)

	# Load the placements and backdrops for this set.
	LoadPlacements("Nursery1")
	LoadBackdrops(pSet)
	...
	# "Nursery1_S.py" file with an Initialize function that creates them.
	try:
		import Nursery1_S
		Nursery1_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Nursery1")

def Terminate():
	App.g_kSetManager.DeleteSet("Nursery1")
...
def LoadPlacements(sSetName):
	...
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(320.0, 9995.0, 990.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.000000, 1.000000, 0.000000)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.000000, 0.000000, 1.000000)
	...
	# Position "Nursery"
	kThis = App.Waypoint_Create("Nursery Gate", sSetName, None)
	kThis.SetStatic(1)
	kThis.SetNavPoint(1)
	kThis.SetTranslateXYZ(0.0, 5500.0, 100.0)
	kForward = App.TGPoint3()
	...
	kThis.Update(0)
	kThis = None
	# End position "Nursery"
	
	# Position "Nursery2"
	kThis = App.Waypoint_Create("Birthing Ground", sSetName, None)
	kThis.SetStatic(1)
	kThis.SetNavPoint(1)
	kThis.SetTranslateXYZ(300.0, 9800.0, 1000.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.208765, 0.512881, 0.832689)
	kUp = App.TGPoint3()
	kUp.SetXYZ(-0.249968, 0.851151, -0.461582)
	...
	kThis.Update(0)
	kThis = None
	# End position "Nursery2"
	
	# Position "Nursery3"
	kThis = App.Waypoint_Create("Hazard Beacon", sSetName, None)
	kThis.SetStatic(1)
	kThis.SetNavPoint(1)
	kThis.SetTranslateXYZ(-100.0, 11000.0, -1500.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.208765, 0.512881, 0.832689)
	kUp = App.TGPoint3()
	kUp.SetXYZ(-0.249968, 0.851151, -0.461582)
	...
	kThis.Update(0)
	kThis = None
	# End position "Nursery3"
	
	# Position "Nursery4"
	kThis = App.Waypoint_Create("Matter Condensation", sSetName, None)
	kThis.SetStatic(1)
	kThis.SetNavPoint(1)
...
def LoadBackdrops(pSet):
	...
	# Backdrop Sphere "Backdrop treknebulabz1"
	kThis = App.BackdropSphere_Create()
	kThis.SetName("Backdrop treknebulabz1")
	kThis.SetTranslateXYZ(0.000000, 0.000000, 0.000000)
	kForward = App.TGPoint3()
	...
	kThis.AlignToVectors(kForward, kUp)
	kThis.SetTextureFileName("data/backgrounds/treknebulabz1.tga")
	kThis.SetTargetPolyCount(256)
	...
	kThis.Rebuild()
	pSet.AddBackdropToSet(kThis,"Backdrop treknebulabz1")
	kThis.Update(0)
	kThis = None
