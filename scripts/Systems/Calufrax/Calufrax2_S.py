###############################################################################
#       Filename:       Calufrax2_S.py
#       
#       Confidential and Proprietary, Copyright 2001 by Totally Games/Ben Howard
#       
#       Creates Calufrax 2 static objects.  Called by Calufrax2.py when region is created
#       
#       Created:        09/17/00 
...
def Initialize(pSet):
        ...
        pNebula.AddNebulaSphere(-48000.0, 76000.0, -2000.0, 1600.0)
        ...
        # Adds a fuzzy sphere at x, y, z (in world coord) of specified size (1000.0 in this case)
        pNebula2.AddNebulaSphere(-46800.0, 75000.0, -1400.0, 800.0)
        # Puts the nebula in the set
        ...
        # Adds a fuzzy sphere at x, y, z (in world coord) of specified size (1000.0 in this case)
        pNebula3.AddNebulaSphere(-50000.0, 77000.0, -2800.0, 3200.0)
        # Puts the nebula in the set
        ...
        # Adds a fuzzy sphere at x, y, z (in world coord) of specified size (1000.0 in this case)
        pNebula4.AddNebulaSphere(-53000.0, 78000.0, -3300.0, 5000.0)
        # Puts the nebula in the set
        pSet.AddObjectToSet(pNebula4, "Nebula4")

        if (App.g_kUtopiaModule.IsHost ()) or not (App.g_kUtopiaModule.IsMultiplayer()):
                # Create ship
                pAlien  = loadspacehelper.CreateShip("KessokHeavy", pSet, "Unknown Alien", "Alien Location")                    
        ...
        if (App.g_kUtopiaModule.IsMultiplayer()):
                ...
                pEnemies.AddName("Unknown Alien")
...
def ResetEnemyFriendlyGroups ():
        ...
        if (pNetwork and pGame):
                ...
                pSet = App.g_kSetManager.GetSet("Calufrax2")
                pAIShip = App.ShipClass_GetObject(pSet, "Unknown Alien")
                if pAIShip != None:    
                   pEnemyGroup.AddName("Unknown Alien")
                   import AttackAI
