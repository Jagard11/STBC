...
def Initialize(pSet):
	...
	if (App.g_kUtopiaModule.IsHost ()) or not (App.g_kUtopiaModule.IsMultiplayer()):
		# Create ship
		pBorgPrime	= loadspacehelper.CreateShip("BorgPrime", pSet, "BorgPrime", "BorgPrime Location")

		pBorgPrime.SetInvincible(1)
		pBorgPrime.SetHurtable(0)
		pBorgPrime.SetTargetable(1)
		pBorgPrime.SetCollisionsOn(1)
		pBorgPrime.SetScale(39.5)
		vCurVelocity = App.TGPoint3()
		vCurVelocity.SetXYZ(0, 0, -0.01)
		pBorgPrime.SetAngularVelocity(vCurVelocity, App.PhysicsObjectClass.DIRECTION_MODEL_SPACE)

		MissionLib.HideSubsystems(pBorgPrime)

		#HEY JB - nice to see you using this :D  lol
		from Custom.Autoload import PlanetShipAddon
		PlanetShipAddon.SetPlanetShipGlowSize(pBorgPrime, 55)
		PlanetShipAddon.SetPlanetShipGlowColor(pBorgPrime, 0.000000, 0.643137, 0.000000, 0.32)
		PlanetShipAddon.SetPlanetShipAtmosphereScript(pBorgPrime, "AtmosphereBorg")
		PlanetShipAddon.SetPlanetShipAtmosphereScaleBonus(pBorgPrime, 0.001)
		PlanetShipAddon.SetPlanetShipAtmosphereAngularVelocity(pBorgPrime, 0, 0, -0.015)
	...
	if (App.g_kUtopiaModule.IsMultiplayer()):
		...
		pBorgPrime.SetAI(SelfDefenseAI.CreateAI(pBorgPrime))
...
def ResetEnemyFriendlyGroups ():
	...
	pBorgPrime = App.ShipClass_GetObject(pSet, "BorgPrime")
	...
	if pBorgPrime != None:
		pNeutralGroup.AddName("BorgPrime")
		pBorgPrime.SetAI(SelfDefenseAI.CreateAI(pBorgPrime))
                if pBorg2 != None:   
                        pBorg2.SetAI(SelfDefenseAI.CreateAI(pBorg2))    
                if pBorg3 != None:
