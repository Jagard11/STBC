...
def Initialize():
	# Create the set ("Cardassia7")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Cardassia7")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Cardassia.Cardassia7")
...
def GetSet():
	return App.g_kSetManager.GetSet("Cardassia7")

def Terminate():
	App.g_kSetManager.DeleteSet("Cardassia7")
...
def LoadPlacements(sSetName):
	...
	kUp.SetXYZ(0.0, 0.0, 1.0)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, 0.75, 0.6, 0.6)
	kThis.Update(0)
	kThis = None
	...
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(-82857142.0, 0.0, 0.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.0, 1.0, 0.0)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.0, 0.0, 1.0)
	...
	kThis.SetStatic(1)
	kThis.SetNavPoint(1)
	kThis.SetTranslateXYZ(-82617000.0, 0.0, 0.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.0, 1.0, 0.0)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.0, 0.0, 1.0)
