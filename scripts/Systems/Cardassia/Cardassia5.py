...
def Initialize():
	# Create the set ("Cardassia5")
	pSet = App.SetClass_Create()

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Cardassia.Cardassia5")

	App.g_kSetManager.AddSet(pSet, "Cardassia5")
	...
	# "Cardassia5_S.py" file with an Initialize function that creates them.
	try:
		import Cardassia5_S
		Cardassia5_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Cardassia5")

def Terminate():
	App.g_kSetManager.DeleteSet("Cardassia5")
...
def LoadPlacements(sSetName):
	...
	kUp.SetXYZ(0.0, 0.0, 1.0)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, 0.75, 0.6, 0.75)
	kThis.Update(0)
	kThis = None
	...
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(-22228571.0, 0.0, 0.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.0, 1.0, 0.0)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.0, 0.0, 1.0)
	...
	kThis.SetStatic(1)
	kThis.SetNavPoint(1)
	kThis.SetTranslateXYZ(-21988000.0, 0.0, 0.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.0, 1.0, 0.0)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.0, 0.0, 1.0)
