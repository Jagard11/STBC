...
def Initialize():
	# Create the set ("Cardassia2")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Cardassia2")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Cardassia.Cardassia2")
	...
	# "Cardassia2_S.py" file with an Initialize function that creates them.
	try:
		import Cardassia2_S
		Cardassia2_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Cardassia2")

def Terminate():
	App.g_kSetManager.DeleteSet("Cardassia2")
...
def LoadPlacements(sSetName):
	...
	kUp.SetXYZ(0.0, 0.0, 1.0)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, 0.75, 0.6, 0.9)
	kThis.Update(0)
	kThis = None
	...
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(-3091428.0, 0.0, 0.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.0, 1.0, 0.0)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.0, 0.0, 1.0)
	...
	kThis.SetStatic(1)
	kThis.SetNavPoint(1)
	kThis.SetTranslateXYZ(-2851000.0, 0.0, 0.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.0, 1.0, 0.0)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.0, 0.0, 1.0)
