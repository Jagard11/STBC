...
def Initialize():
	# Create the set ("Cardassia4")
	pSet = App.SetClass_Create()

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Cardassia.Cardassia4")

	App.g_kSetManager.AddSet(pSet, "Cardassia4")
	...
	# "Cardassia4_S.py" file with an Initialize function that creates them.
	try:
		import Cardassia4_S
		Cardassia4_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Cardassia4")

def Terminate():
	App.g_kSetManager.DeleteSet("Cardassia4")
...
def LoadPlacements(sSetName):
	...
	kUp.SetXYZ(0.0, 0.0, 1.0)
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, 0.75, 0.6, 0.8)
	kThis.Update(0)
	kThis = None
	...
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(-6514285.0, 0.0, 0.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.0, 1.0, 0.0)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.0, 0.0, 1.0)
	...
	kThis.SetStatic(1)
	kThis.SetNavPoint(1)
	kThis.SetTranslateXYZ(-6274000.0, 0.0, 0.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.0, 1.0, 0.0)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.0, 0.0, 1.0)
