...
def Initialize():
	...
	App.g_kSetManager.AddSet(pSet, "Sol4")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Sol.Sol4")
	...
	# "Sol4_S.py" file with an Initialize function that creates them.
	try:
		import Sol4_S
		Sol4_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	...
	return App.g_kSetManager.GetSet("Sol4")

def Terminate():
	debug(__name__ + ", Terminate")
	App.g_kSetManager.DeleteSet("Sol4")

def LoadPlacements(sSetName):
	debug(__name__ + ", LoadPlacements")
	placer = Systems.FoundationUtils.SystemPlacer(388.23, 3, 3)

	# Position "Planet1"
	kThis = App.Waypoint_Create("Mars", sSetName, None)
	kThis.SetStatic(1)
	...
	# Position "Planet1"
	kThis = App.Waypoint_Create("Phobos", sSetName, None)
	kThis.SetStatic(1)
	coord = placer.SetOnOrbit()
	kThis.SetTranslateXYZ(coord[0], coord[1], coord[2])
	kForward = App.TGPoint3()
	...
	# Position "Planet1"
	kThis = App.Waypoint_Create("Deimos", sSetName, None)
	kThis.SetStatic(1)
	coord = placer.SetOnOrbit()
	kThis.SetTranslateXYZ(coord[0], coord[1], coord[2])
	kForward = App.TGPoint3()
	...
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(0.0, -13028571.43, -3000.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.000000, 1.000000, 0.000000)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.000000, 0.000000, 1.000000)
