...
def Initialize():
	...
	App.g_kSetManager.AddSet(pSet, "Sol1")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Sol.Sol1")
	...
	# "Sol1_S.py" file with an Initialize function that creates them.
	try:
		import Sol1_S
		Sol1_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	...
	return App.g_kSetManager.GetSet("Sol1")

def Terminate():
	debug(__name__ + ", Terminate")
	App.g_kSetManager.DeleteSet("Sol1")

def LoadPlacements(sSetName):
	debug(__name__ + ", LoadPlacements")
	placer = Systems.FoundationUtils.SystemPlacer(278.80, 4, 4)

	# Position "Planet1"
	kThis = App.Waypoint_Create("Mercury", sSetName, None)
	kThis.SetStatic(1)
	...
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(0.0, -3308571.43, -3000.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.000000, 1.000000, 0.000000)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.000000, 0.000000, 1.000000)
