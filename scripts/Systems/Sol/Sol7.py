...
def Initialize():
	...
	App.g_kSetManager.AddSet(pSet, "Sol7")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Sol.Sol7")
	...
	# "Sol7_S.py" file with an Initialize function that creates them.
	try:
		import Sol7_S
		Sol7_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	...
	return App.g_kSetManager.GetSet("Sol7")

def Terminate():
	debug(__name__ + ", Terminate")
	App.g_kSetManager.DeleteSet("Sol7")

def LoadPlacements(sSetName):
	debug(__name__ + ", LoadPlacements")
	placer = Systems.FoundationUtils.SystemPlacer(2921.03, 2, 2)

	# Position "Planet1"
	kThis = App.Waypoint_Create("Uranus", sSetName, None)
	kThis.SetStatic(1)
	...
	# Position "Planet1" - 436,270 km average average distance
	kThis = App.Waypoint_Create("Ariel", sSetName, None)
	kThis.SetStatic(1)
	coord = placer.SetOnOrbit(10928.00)
	kThis.SetTranslateXYZ(coord[0], coord[1], coord[2])
	kForward = App.TGPoint3()
	...
	# Position "Planet1" - 436,270 km average average distance
	kThis = App.Waypoint_Create("Umbriel", sSetName, None)
	kThis.SetStatic(1)
	coord = placer.SetOnOrbit(15198.29)
	kThis.SetTranslateXYZ(coord[0], coord[1], coord[2])
	kForward = App.TGPoint3()
	coord = placer.SetFacing()
	kForward.SetXYZ(coord[0], coord[1], coord[2])
	...
	# Position "Planet1" - 436,270 km average average distance
	kThis = App.Waypoint_Create("Titania", sSetName, None)
	kThis.SetStatic(1)
	coord = placer.SetOnOrbit(24929.71)
	kThis.SetTranslateXYZ(coord[0], coord[1], coord[2])
	kForward = App.TGPoint3()
	coord = placer.SetFacing()
	kForward.SetXYZ(coord[0], coord[1], coord[2])
	...
	# Position "Planet1" - 436,270 km average average distance
	kThis = App.Waypoint_Create("Oberon", sSetName, None)
	kThis.SetStatic(1)
	coord = placer.SetOnOrbit(33291.43)
	kThis.SetTranslateXYZ(coord[0], coord[1], coord[2])
	kForward = App.TGPoint3()
	coord = placer.SetFacing()
	kForward.SetXYZ(coord[0], coord[1], coord[2])
	...
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(0.0, -165714285.71, -3000.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.000000, 1.000000, 0.000000)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.000000, 0.000000, 1.000000)
