...
def Initialize():
	# Create the set ("Borealis1")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Borealis1")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Borealis.Borealis1")

	# Activate the proximity manager for our set.
	pSet.SetProximityManagerActive(1)

	# Load the placements and backdrops for this set.
	LoadPlacements("Borealis1")
	LoadBackdrops(pSet)
	...
	# "Borealis1_S.py" file with an Initialize function that creates them.
	try:
		import Borealis1_S
		Borealis1_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Borealis1")

def Terminate():
	App.g_kSetManager.DeleteSet("Borealis1")
...
def LoadPlacements(sSetName):
	...
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(-65.0, 70.0, 2.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.000000, 1.000000, 0.000000)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.000000, 0.000000, 1.000000)
	...
	kThis = App.Waypoint_Create("1", sSetName, None)
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(1000.0, 1000.0, 0.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.000000, 1.000000, 0.000000)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.000000, 0.000000, 1.000000)
	...
	# Position "Station Location"
	kThis = App.Waypoint_Create("2", sSetName, None)
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(0.0, 1000.0, 0.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.000000, 1.000000, 0.000000)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.000000, 0.000000, 1.000000)
