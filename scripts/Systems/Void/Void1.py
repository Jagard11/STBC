...
def Initialize():
	# Create the set ("Void1")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Void1")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Void.Void1")

	# Activate the proximity manager for our set.
	pSet.SetProximityManagerActive(1)

	# Load the placements and backdrops for this set.
	LoadPlacements("Void1")
	LoadBackdrops(pSet)
	...
	# "Void1_S.py" file with an Initialize function that creates them.
	try:
		import Void1_S
		Void1_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Void1")

def Terminate():
	App.g_kSetManager.DeleteSet("Void1")
...
def LoadPlacements(sSetName):
	...
	kThis.SetStatic(1)
	kThis.SetTranslateXYZ(22502.0, -22005.0, -13202.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.000000, 1.000000, 0.000000)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.000000, 0.000000, 1.000000)
	...
	# Position "Nav Point"
	kThis = App.Waypoint_Create("Nav Point", sSetName, None)
	kThis.SetStatic(1)
	kThis.SetNavPoint(1)
	kThis.SetTranslateXYZ(-2500.0, 12000.0, 320.0)
	kForward = App.TGPoint3()
	...
	# Position "NavPoint"
	kThis = App.Waypoint_Create("NavPoint", sSetName, None)
	kThis.SetStatic(1)
	kThis.SetNavPoint(1)
	kThis.SetTranslateXYZ(-5500.0, -3000.0, -5200.0)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.208765, 0.512881, 0.832689)
	kUp = App.TGPoint3()
	kUp.SetXYZ(-0.249968, 0.851151, -0.461582)
	...
	kThis.Update(0)
	kThis = None
	# End position "NavPoint"

	# Position "Nav Pt"
	kThis = App.Waypoint_Create("Nav Pt", sSetName, None)
	kThis.SetStatic(1)
	kThis.SetNavPoint(1)
