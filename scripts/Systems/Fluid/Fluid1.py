...
def Initialize():
	# Create the set ("Fluid1")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Fluid1")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Fluid.Fluid1")

	# Activate the proximity manager for our set.
	pSet.SetProximityManagerActive(1)

	# Load the placements and backdrops for this set.
	LoadPlacements("Fluid1")
	LoadBackdrops(pSet)
	...
	# "Fluid1_S.py" file with an Initialize function that creates them.
	try:
		import Fluid1_S
		Fluid1_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Fluid1")

def Terminate():
	App.g_kSetManager.DeleteSet("Fluid1")
