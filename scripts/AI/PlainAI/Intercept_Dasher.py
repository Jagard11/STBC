...
class Intercept_Dasher(BaseAI.BaseAI):
	...
	def LostFocus(self):
		...
		if pShip:
			AI.FoundationAI.DisableInterceptSpeed(pShip)
	...
	def GetNextUpdateTime(self):
		...
		# return max(min(0.4 * self.ratio, 2.0), 0.3)
...
#			debug("Intercept update done: no set")
		...
		if (pObject != None):
			...
			if fMaxSpeed > 0:
				...
				# Something weird goes on with fDistance at warp; the value is rather
				# unpredictable.  This is intended to correct that.
				fRealDistance = fObjectDistance - (pObject.GetRadius() * 1.1)
				pPlanet = App.Planet_Cast(pObject)
				if pPlanet:
					fRealDistance = fRealDistance - (pPlanet.GetAtmosphereRadius() * 1.5)

				# If we're extremely far away, override the normal
				# speed limitations and do an in-set warp.
				bWarping = 0
				if self.fMaximumSpeed == 1.0e20 and fRealDistance > 425.0:
					bWarping = 1

				fCurrentSpeed = pShip.GetVelocityTG().Length()

				# While we want to allow real course changes while warping, if we haven't gotten start
				# yet we want to limit the angle at which higher speeds can be initiated.
				if fCurrentSpeed < 40 and fAngle > 0.2:
					bWarping = 0

				if bWarping:	AI.FoundationAI.EnableInterceptSpeed(pShip)
				else:			AI.FoundationAI.DisableInterceptSpeed(pShip)
				...
				if fCurrentSpeed:
					...
					if bWarping:
						# if fRealDistance < 450:
						# 	pShip.InSystemWarp(pObject, self.fInSystemWarpDistance)
						# else:
						fStopDist = AI.FoundationAI.DecelerationDistance(fCurrentSpeed, fMaxAccel)
                                                # Defiant: test to lower!
						self.ratio = fRealDistance / fStopDist / 2
						if self.ratio < 1.0:
							self.ratio = self.ratio * 0.2

						fSpeed = fCurrentSpeed * self.ratio
				...
				fSpeed = max(min(fSpeed, self.fMaximumSpeed), 0.0)
