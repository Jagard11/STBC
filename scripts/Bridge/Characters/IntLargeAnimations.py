...
####################################################
# Intrepid-Bridge Animations
####################################################

	# Place at T
def IntPlaceAtT(pCharacter):
#	print("Place T")
	kAM = App.g_kAnimationManager
	...
	pSequence = App.TGSequence_Create()
	pAnimNode = pCharacter.GetAnimNode()
	pAction = App.TGAnimPosition_Create(pAnimNode, "Int_stand_t_l")
	pSequence.AddAction(pAction)
	return pSequence
...
def IntPlaceAtL1(pCharacter):
	kAM = App.g_kAnimationManager
	kAM.LoadAnimation ("data/animations/Int_L1toG3_L.nif", "eb_L1toG3_L")
	pCharacter = App.CharacterClass_Cast(pCharacter)
	pSequence = App.TGSequence_Create()
	pAnimNode = pCharacter.GetAnimNode()
	pAction = App.TGAnimPosition_Create(pAnimNode, "eb_L1toG3_L")
	pSequence.AddAction(pAction)
	return pSequence

	# Seat at T
def IntSeatedT(pCharacter):
#	print("Seated T")
	kAM = App.g_kAnimationManager
	...
	pSequence = App.TGSequence_Create()
	pAnimNode = pCharacter.GetAnimNode()
	pAction = App.TGAnimAction_Create(pAnimNode, "Int_seated_t_l", 0, 0, 1)
	pSequence.AddAction(pAction)
	return pSequence

	# Turn T towards captain large
def IntTurnAtTTowardsCaptain(pCharacter):
#	print("Turn T")
	kAM = App.g_kAnimationManager
	...
	pOpenEyes = Bridge.Characters.CommonAnimations.EyesOpenMouthClosed(pCharacter)
	pSequence.AddAction(pOpenEyes)
	pSequence.AddAction(App.TGAnimAction_Create(pCharacter.GetAnimNode(), "Int_face_capt_t", 0, 0, 1), pOpenEyes)
	return pSequence

	# Turn back T from looking at captain
def IntTurnBackAtTFromCaptain(pCharacter):
#	print("Turn Back T")
	kAM = App.g_kAnimationManager
	...
	pOpenEyes = Bridge.Characters.CommonAnimations.EyesOpenMouthClosed(pCharacter)
	pSequence.AddAction(pOpenEyes)
	pSequence.AddAction(App.TGAnimAction_Create(pCharacter.GetAnimNode(), "Int_face_capt_t_reverse", 0, 0, 1), pOpenEyes)
	return pSequence

	# lean left animation
def IntTHit(pCharacter):
	return None
	#print("Hit T - animation doesn't exist")
	kAM = App.g_kAnimationManager
	...
	# Lean
	pLeanAction = App.TGAnimAction_Create(pAnimNode, "Int_hit_t", 0, 0)
	pSequence.AddAction(pLeanAction)
...
#			pSequence.AddAction(App.TGSoundAction_Create("FemaleEek"+ str(App.g_kSystemWrapper.GetRandomNumber(7)+1)))
	...
	# lean hard left animation
def IntTHitHard(pCharacter):
	return IntTHit (pCharacter)
...
def IntTConsoleInteraction(pCharacter):
	pCharacter = App.CharacterClass_Cast(pCharacter)
	if not (pCharacter):
		return None
