##############################################################################
#	Filename:	novabridge.py
#	
#	Confidential and Proprietary, Copyright 2000 by Totally Games
#	
#	This contains the code to create and configure the nova class bridge.
#	It is only called by LoadBridge.Initialize("novabridge"), so don't
...
#debug = App.CPyDebug(__name__).Prnova
#debug("Loading " + __name__ + " module...")

###############################################################################
#	CreateBridgeModel()
#
#	Create the nova bridge model, viewscreen and main camera, attaching them
...
def CreateBridgeModel(pBridgeSet):
	...
	pcEnvPath = "data/Models/Sets/novabridge/" + pcDetail[iDetail]

	# Pre-load the Bridge model and viewscreen
	App.g_kModelManager.LoadModel("data/Models/Sets/novabridge/novabridge.nif", None, pcEnvPath)
	App.g_kModelManager.LoadModel("data/Models/Sets/novabridge/novaViewScreen.NIF", None, pcEnvPath)

	# Load the viewscreen and set it up specially with SetViewScreen()
	pViewScreen = App.ViewScreenObject_Create("data/Models/Sets/novabridge/novaViewScreen.NIF")
	pBridgeSet.SetViewScreen(pViewScreen, "viewscreen")

	# Setup bridge object
	pBridgeObject = App.BridgeObjectClass_Create("data/Models/Sets/novabridge/novabridge.nif")
	...
	# Load novaetheus bridge specific sounds
	LoadSounds()
...
def ConfigureCharacters(pBridgeSet):
	...
	import Bridge.Characters.novaFelix
	import Bridge.Characters.novaKiska
	import Bridge.Characters.novaSaffi
	import Bridge.Characters.novaBrex
	import Bridge.Characters.novaMiguel

	import Bridge.Characters.novaFemaleExtra1
	import Bridge.Characters.novaFemaleExtra2
	import Bridge.Characters.novaFemaleExtra3
	import Bridge.Characters.novaMaleExtra1
	import Bridge.Characters.novaMaleExtra2
	import Bridge.Characters.novaMaleExtra3

	pnovaFelix = App.CharacterClass_Cast(pBridgeSet.GetObject("Tactical"))
	pnovaKiska = App.CharacterClass_Cast(pBridgeSet.GetObject("Helm"))
	pnovaSaffi = App.CharacterClass_Cast(pBridgeSet.GetObject("XO"))
	pnovaMiguel = App.CharacterClass_Cast(pBridgeSet.GetObject("Science"))
	pnovaBrex = App.CharacterClass_Cast(pBridgeSet.GetObject("Engineer"))

	pnovaFemaleExtra1 = App.CharacterClass_Cast(pBridgeSet.GetObject("FemaleExtra1"))
	pnovaFemaleExtra2 = App.CharacterClass_Cast(pBridgeSet.GetObject("FemaleExtra2"))
	pnovaFemaleExtra3 = App.CharacterClass_Cast(pBridgeSet.GetObject("FemaleExtra3"))

	pnovaMaleExtra1 = App.CharacterClass_Cast(pBridgeSet.GetObject("MaleExtra1"))
	pnovaMaleExtra2 = App.CharacterClass_Cast(pBridgeSet.GetObject("MaleExtra2"))
	pnovaMaleExtra3 = App.CharacterClass_Cast(pBridgeSet.GetObject("MaleExtra3"))

	Bridge.Characters.novaFelix.ConfigureForPrometheus(pnovaFelix)
	Bridge.Characters.novaKiska.ConfigureForPrometheus(pnovaKiska)
	Bridge.Characters.novaSaffi.ConfigureForPrometheus(pnovaSaffi)
	Bridge.Characters.novaMiguel.ConfigureForPrometheus(pnovaMiguel)
	Bridge.Characters.novaBrex.ConfigureForPrometheus(pnovaBrex)

	if (pnovaFemaleExtra1):
		Bridge.Characters.novaFemaleExtra1.ConfigureForPrometheus(pnovaFemaleExtra1)
	if (pnovaFemaleExtra2):
		Bridge.Characters.novaFemaleExtra2.ConfigureForPrometheus(pnovaFemaleExtra2)
	if (pnovaFemaleExtra3):
		Bridge.Characters.novaFemaleExtra3.ConfigureForPrometheus(pnovaFemaleExtra3)
	if (pnovaMaleExtra1):
		Bridge.Characters.novaMaleExtra1.ConfigureForPrometheus(pnovaMaleExtra1)
	if (pnovaMaleExtra2):
		Bridge.Characters.novaMaleExtra2.ConfigureForPrometheus(pnovaMaleExtra2)
	if (pnovaMaleExtra3):
		Bridge.Characters.novaMaleExtra3.ConfigureForPrometheus(pnovaMaleExtra3)

	pCamera = App.ZoomCameraObjectClass_GetObject(pBridgeSet, "maincamera")
	pCamera.SetTranslateXYZ(-35, 85, -3)


###############################################################################
#	LoadSounds()
#
#	Load any novaetheus bridge specific sounds
#
#	Args:	none
#
#	Return:	none
...
#	debug("Loading novaetheus door sound")
...
#	UnloadSounds()
#
#	Unload any novaetheus bridge specific sounds
#
#	Args:	none
#
#	Return:	none
...
#	PreloadAnimations()
#
#	Load any novaetheus bridge specific animations that are common
#
#	Args:	none
#
#	Return:	none
...
def PreloadAnimations ():
	...
	kAM.LoadAnimation ("data/animations/nova_door_L1.nif", "nova_Door_L1")
	kAM.LoadAnimation ("data/animations/nova_door_L2.nif", "nova_Door_L2")

	# Small animations
	# Science Movement
	kAM.LoadAnimation ("data/animations/nova_stand_s_s.nif", "nova_stand_s_s")
	kAM.LoadAnimation ("data/animations/nova_seated_s_s.nif", "nova_seated_s_s")
	kAM.LoadAnimation ("data/animations/nova_face_capt_s.nif", "nova_face_capt_s")
	kAM.LoadAnimation ("data/animations/nova_chair_s_face_capt.nif", "nova_chair_s_face_capt")
	kAM.LoadAnimation ("data/animations/nova_face_capt_s_reverse.nif", "nova_face_capt_s_reverse")
	kAM.LoadAnimation ("data/animations/nova_chair_s_face_capt_reverse.nif", "nova_chair_s_face_capt_reverse")
	...
	# Engineer Movement
	kAM.LoadAnimation ("data/animations/nova_stand_e_s.nif", "nova_stand_e_s")
	kAM.LoadAnimation ("data/animations/nova_seated_e_s.nif", "nova_seated_e_s")
	kAM.LoadAnimation ("data/animations/nova_face_capt_e.nif", "nova_face_capt_e")
	kAM.LoadAnimation ("data/animations/nova_chair_e_face_capt.nif", "nova_chair_e_face_capt")
	kAM.LoadAnimation ("data/animations/nova_face_capt_e_reverse.nif", "nova_face_capt_e_reverse")
	kAM.LoadAnimation ("data/animations/nova_chair_e_face_capt_reverse.nif", "nova_chair_e_face_capt_reverse")
	...
	# medium animations
	# Helm Movement
	kAM.LoadAnimation ("data/animations/nova_stand_h_m.nif", "nova_stand_h_m")
	...
	kAM.LoadAnimation ("data/animations/Int_chair_H_face_capt.nif", "Int_chair_H_face_capt")
	kAM.LoadAnimation ("data/animations/Int_face_capt_h_reverse.nif", "Int_face_capt_h_reverse")
	kAM.LoadAnimation ("data/animations/Int_chair_H_face_capt_reverse.nif", "Int_chair_H_face_capt_reverse")
	kAM.LoadAnimation ("data/animations/EB_glance_h_m.nif", "Int_glance_h_m")
	kAM.LoadAnimation ("data/animations/EB_glance_h_m_reverse.nif", "Int_glance_h_m_reverse")
	...
	kAM.LoadAnimation ("data/animations/Int_seatedm_C_M.nif", "Int_seatedm_c_m")
	...
	# Extras
	kAM.LoadAnimation ("data/animations/nova_L1toG3_S.nif", "nova_L1toG3_S")
	kAM.LoadAnimation ("data/animations/nova_L1toG3_M.nif", "nova_L1toG3_M")
	kAM.LoadAnimation ("data/animations/nova_L1toG3_L.nif", "nova_L1toG3_L")

	kAM.LoadAnimation ("data/animations/nova_L2toG1_S.nif", "nova_L2toG1_S")
	kAM.LoadAnimation ("data/animations/nova_L2toG1_M.nif", "nova_L2toG1_M")
	kAM.LoadAnimation ("data/animations/nova_L2toG1_L.nif", "nova_L2toG1_L")

	kAM.LoadAnimation ("data/animations/nova_L2toG2_S.nif", "nova_L2toG2_S")
	kAM.LoadAnimation ("data/animations/nova_L2toG2_M.nif", "nova_L2toG2_M")
	kAM.LoadAnimation ("data/animations/nova_L2toG2_L.nif", "nova_L2toG2_L")

	kAM.LoadAnimation ("data/animations/nova_G1toL2_S.nif", "nova_G1toL2_S")
	kAM.LoadAnimation ("data/animations/nova_G1toL2_M.nif", "nova_G1toL2_M")
	kAM.LoadAnimation ("data/animations/nova_G1toL2_L.nif", "nova_G1toL2_L")
	
	kAM.LoadAnimation ("data/animations/nova_G2toL2_S.nif", "nova_G2toL2_S")
	kAM.LoadAnimation ("data/animations/nova_G2toL2_M.nif", "nova_G2toL2_M")
	kAM.LoadAnimation ("data/animations/nova_G2toL2_L.nif", "nova_G2toL2_L")
	
	kAM.LoadAnimation ("data/animations/nova_G3toL1_S.nif", "nova_G3toL1_S")
	kAM.LoadAnimation ("data/animations/nova_G3toL1_M.nif", "nova_G3toL1_M")
	kAM.LoadAnimation ("data/animations/nova_G3toL1_L.nif", "nova_G3toL1_L")


	# Large animations
	# Tactical Movement
	kAM.LoadAnimation ("data/animations/nova_stand_t_l.nif", "nova_stand_t_l")
	kAM.LoadAnimation ("data/animations/nova_seated_t_l.nif", "nova_seated_t_l")
	kAM.LoadAnimation ("data/animations/nova_seatedm_t_l.nif", "nova_seatedm_t_l")
	kAM.LoadAnimation ("data/animations/nova_face_capt_t.nif", "nova_face_capt_t")
	kAM.LoadAnimation ("data/animations/nova_chair_T_face_capt.nif", "nova_chair_T_face_capt")
	kAM.LoadAnimation ("data/animations/nova_face_capt_t_reverse.nif", "nova_face_capt_t_reverse")
	kAM.LoadAnimation ("data/animations/nova_chair_T_face_capt_reverse.nif", "nova_chair_T_face_capt_reverse")
	kAM.LoadAnimation ("data/animations/EB_hit_t.NIF", "nova_hit_t")
...
#	UnloadAnimations()
#
#	Unload any novaetheus bridge specific animations that are common
#
#	Args:	none
#
#	Return:	none
...
def UnloadAnimations ():
	...
	kAM.FreeAnimation("nova_Door_L1")
	kAM.FreeAnimation("nova_Door_L2")

	# Small animations
	# Science Movement
	kAM.FreeAnimation("nova_stand_s_s")
	kAM.FreeAnimation("nova_seated_s_s")
	kAM.FreeAnimation("nova_face_capt_s")
	kAM.FreeAnimation("nova_chair_s_face_capt")
	kAM.FreeAnimation("nova_face_capt_s_reverse")
	kAM.FreeAnimation("nova_chair_s_face_capt_reverse")
	...
	# Engineer Movement
	kAM.FreeAnimation("nova_stand_e_s")
	kAM.FreeAnimation("nova_seated_e_s")
	kAM.FreeAnimation("nova_face_capt_e")
	kAM.FreeAnimation("nova_chair_e_face_capt")
	kAM.FreeAnimation("nova_face_capt_e_reverse")
	kAM.FreeAnimation("nova_chair_e_face_capt_reverse")
	...
	# medium animations
	# Helm Movement
	kAM.FreeAnimation("nova_stand_h_m")
	...
	kAM.FreeAnimation("Int_seatedm_c_m")
	...
	#Extra
	kAM.FreeAnimation("nova_L1toG3_S")
	kAM.FreeAnimation("nova_L1toG3_M")
	kAM.FreeAnimation("nova_L1toG3_L")

	kAM.FreeAnimation("nova_L2toG1_S")
	kAM.FreeAnimation("nova_L2toG1_M")
	kAM.FreeAnimation("nova_L2toG1_L")

	kAM.FreeAnimation("nova_L2toG2_S")
	kAM.FreeAnimation("nova_L2toG2_M")
	kAM.FreeAnimation("nova_L2toG2_L")

	kAM.FreeAnimation("nova_G1toL2_S")
	kAM.FreeAnimation("nova_G1toL2_M")
	kAM.FreeAnimation("nova_G1toL2_L")
	
	kAM.FreeAnimation("nova_G2toL2_S")
	kAM.FreeAnimation("nova_G2toL2_M")
	kAM.FreeAnimation("nova_G2toL2_L")
	
	kAM.FreeAnimation("nova_G3toL1_S")
	kAM.FreeAnimation("nova_G3toL1_M")
	kAM.FreeAnimation("nova_G3toL1_L")
	
	# Large animations
	# Tactical Movement
	kAM.FreeAnimation("nova_stand_t_l")
	kAM.FreeAnimation("nova_seated_t_l")
	kAM.FreeAnimation("nova_seatedm_t_l")
	kAM.FreeAnimation("nova_face_capt_t")
	kAM.FreeAnimation("nova_chair_T_face_capt")
	kAM.FreeAnimation("nova_face_capt_t_reverse")
	kAM.FreeAnimation("nova_chair_T_face_capt_reverse")
	kAM.FreeAnimation("nova_hit_t")
