...
class FiveSecsTempPhaser(FoundationTech.TechDef):
	def __init__(self, name):
		...
		#print "Initialized FiveSecsTempPhaser"

	def Attach(self, pInstance):
		pShip = App.ShipClass_Cast(App.TGObject_GetTGObjectPtr(pInstance.pShipID))
		if pShip != None:
			pInstance.__dict__['FiveSecsTempPhaser'] = {}
			dMasterDict = pInstance.__dict__['FiveSecsTempPhaser']
		else:
			pass
			#print "FiveSecsTempPhaser Error (at Attach): couldn't acquire ship of id", pInstance.pShipID
	...
	def Detach(self, pInstance):
		...
		if pShip != None:
			...
			#print "FiveSecsTempPhaser Error (at Detach): couldn't acquire ship of id", pInstance.pShipID
			pass
	...
	def PhaserStartedHitting(self, pEvent):
		...
		try:
			...
			return
		if not pInstance.__dict__.has_key("FiveSecsTempPhaser"):
		...
		if not pInstance.__dict__['FiveSecsTempPhaser'].has_key(sTargetName):
			pInstance.__dict__['FiveSecsTempPhaser'][sTargetName] = {"HittingNames": [], "Target": pTarget}
			...
			pTimer.SetTimerStart( App.g_kUtopiaModule.GetGameTime()+3.0 )
			pTimer.SetDelay(0)
			pTimer.SetDuration(0)
			pTimer.SetEvent(pEvent)
			App.g_kTimerManager.AddTimer(pTimer)

			pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]['Timer'] = pTimer
		...
		if not pPhaser.GetName() in pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]['HittingNames']:
			pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]['HittingNames'].append( pPhaser.GetName() )
	...
	def PhaserStoppedHitting(self, pEvent):
		...
		if not pInstance.__dict__['FiveSecsTempPhaser'].has_key(sTargetName):
			...
			return
		if pPhaser.GetName() in pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]['HittingNames']:
			pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]['HittingNames'].remove( pPhaser.GetName() )
			#print "FSGP (Stop): removing phaser", pPhaser.GetName(), "from target hitting list."

		if len(pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]['HittingNames']) <= 0:
			#5-secs hitting sequence aborted... cancel it.
			App.g_kTimerManager.DeleteTimer(pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]['Timer'].GetObjID())
			pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]['Timer'] = None
			del pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]
	...
	def HandleInstaKill(self, pEvent):
		...
		if not pInstance.__dict__.has_key("FiveSecsTempPhaser"):
			...
			return
		if pInstance.__dict__['FiveSecsTempPhaser'].has_key(sTargetName):
			#okydokey, we did it. Blow up the ship. then delete our data about it.
			pTarget = pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]['Target']
			pSet = pTarget.GetContainingSet()
			pSet.RemoveObjectFromSet(pTarget.GetName())
			App.g_kTimerManager.DeleteTimer(pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]['Timer'].GetObjID())
			pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]['Timer'] = None
			del pInstance.__dict__['FiveSecsTempPhaser'][sTargetName]
...
oFiveSecsTempPhaser = FiveSecsTempPhaser("FiveSecsTempPhaser")