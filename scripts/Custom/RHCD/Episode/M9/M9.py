...
def Initialize(pMission):
	...
	g_pMissionTGL = App.g_kLocalizationManager.Load("data/TGL/RHCD/Episode/M9.tgl")
...
def CreateRegions():
	...
	import M9Yiles2_P
	M9Yiles2_P.LoadPlacements(pYiles2Set.GetName())
...
def AddObjectives(pAction):
	...
	# Create a Timer that triggers the release of Warbird #1
	fStartTime = App.g_kUtopiaModule.GetGameTime()
	fRandomTime = App.g_kSystemWrapper.GetRandomNumber(300) + 240
	...
	# Create a Timer that triggers the release of Warbird #2
	fStartTime = App.g_kUtopiaModule.GetGameTime()
	fRandomTime = App.g_kSystemWrapper.GetRandomNumber(300) + 240
	...
	# Create a Timer that triggers the release of Warbird #3
	fStartTime = App.g_kUtopiaModule.GetGameTime()
	fRandomTime = App.g_kSystemWrapper.GetRandomNumber(300) + 240
	...
	# Create a Timer that triggers the release of Warbird #4
	fStartTime = App.g_kUtopiaModule.GetGameTime()
	fRandomTime = App.g_kSystemWrapper.GetRandomNumber(300) + 240
...
def Yiles2Cutscene():
	...
	# Create the Romulans We won't assign their AI just yet, because we don't 
	# want them to activate until their random timer has expired.
	pWarbird1 = loadspacehelper.CreateShip("Warbird", pYiles2Set, "Warbird 1", "Warbird 1 Start")
	pWarbird2 = loadspacehelper.CreateShip("Warbird", pYiles2Set, "Warbird 2", "Warbird 2 Start")
	pWarbird3 = loadspacehelper.CreateShip("Warbird", pYiles2Set, "Warbird 3", "Warbird 3 Start")
	pWarbird4 = loadspacehelper.CreateShip("Warbird", pYiles2Set, "Warbird 4", "Warbird 4 Start")

	# Enemy ships are at Green Alert status.
	pWarbird1.SetAlertLevel(App.ShipClass.GREEN_ALERT)
	pWarbird2.SetAlertLevel(App.ShipClass.GREEN_ALERT)
	pWarbird3.SetAlertLevel(App.ShipClass.GREEN_ALERT)
	pWarbird4.SetAlertLevel(App.ShipClass.GREEN_ALERT)
...
def Activate1(pObject, pEvent):
	...
	pPlayer = App.Game_GetCurrentPlayer()
	pSet = pPlayer.GetContainingSet()
	pShip = App.ShipClass_Cast(App.ShipClass_GetObject(pSet, "Warbird 1"))
	if (pShip != None) and (not pShip.IsDying()) and (not pShip.IsDead()):
		pBridge = App.g_kSetManager.GetSet("bridge")
		pFelix = App.CharacterClass_GetObject(pBridge, "Tactical")
		pSeq = App.TGSequence_Create()	
		...
		# Assign AI
		import RomulanAI
		pWarbird1 = App.ShipClass_GetObject(pYiles2Set, "Warbird 1")
		pWarbird1.SetAI(RomulanAI.CreateAI(pWarbird1))

###############################################################################
#	Activate2()
#
#	Activate AI for Warbird #2
#
#	Args:	pObject	- TGObject object
#		pEvent	- TGEvent object
#
...
def Activate2(pObject, pEvent):
	...
	pPlayer = App.Game_GetCurrentPlayer()
	pSet = pPlayer.GetContainingSet()
	pShip = App.ShipClass_Cast(App.ShipClass_GetObject(pSet, "Warbird 2"))
	if (pShip != None) and (not pShip.IsDying()) and (not pShip.IsDead()):	
		pBridge = App.g_kSetManager.GetSet("bridge")
		pFelix = App.CharacterClass_GetObject(pBridge, "Tactical")
		pSeq = App.TGSequence_Create()	
		...
		# Assign AI
		import RomulanAI
		pWarbird2 = App.ShipClass_GetObject(pYiles2Set, "Warbird 2")
		pWarbird2.SetAI(RomulanAI.CreateAI(pWarbird2))

###############################################################################
#	Activate3()
#
#	Activate AI for Warbird #3
#
#	Args:	pObject	- TGObject object
#		pEvent	- TGEvent object
#
...
def Activate3(pObject, pEvent):
	...
	pPlayer = App.Game_GetCurrentPlayer()
	pSet = pPlayer.GetContainingSet()
	pShip = App.ShipClass_Cast(App.ShipClass_GetObject(pSet, "Warbird 3"))
	if (pShip != None) and (not pShip.IsDying()) and (not pShip.IsDead()):	
		pBridge = App.g_kSetManager.GetSet("bridge")
		pFelix = App.CharacterClass_GetObject(pBridge, "Tactical")
		pSeq = App.TGSequence_Create()	
		...
		# Assign AI
		import RomulanAI
		pWarbird3 = App.ShipClass_GetObject(pYiles2Set, "Warbird 3")
		pWarbird3.SetAI(RomulanAI.CreateAI(pWarbird3))
	
###############################################################################
#	Activate4()
#
#	Activate AI for Warbird #4
#
#	Args:	pObject	- TGObject object
#		pEvent	- TGEvent object
#
...
def Activate4(pObject, pEvent):
	...
	pPlayer = App.Game_GetCurrentPlayer()
	pSet = pPlayer.GetContainingSet()
	pShip = App.ShipClass_Cast(App.ShipClass_GetObject(pSet, "Warbird 4"))
	if (pShip != None) and (not pShip.IsDying()) and (not pShip.IsDead()):	
		pBridge = App.g_kSetManager.GetSet("bridge")
		pFelix = App.CharacterClass_GetObject(pBridge, "Tactical")
		pSeq = App.TGSequence_Create()	
		...
		# Assign AI
		import RomulanAI
		pWarbird4 = App.ShipClass_GetObject(pYiles2Set, "Warbird 4")
		pWarbird4.SetAI(RomulanAI.CreateAI(pWarbird4))	
