...
### Lost_Jedi 2006
######
# handles creation, update and deletion of star streaks.

######################################################################
# Modified by USS Frontier to use in Galaxy Charts
######################################################################
...
#########################
## Constants
C_MAXWARP                       = float(10.0)                            # max warp speed
C_MAX_PULSEUPDATE               = float(4.0)                             # how often to generate a new pulse
C_PULSEUPDATESCALEFACTOR        = float(C_MAX_PULSEUPDATE / C_MAXWARP)   # how often to generate a new streak
C_MAXSTREAKSPERPULSE            = 50                                     # max number of streaks created per pulse
MAX_STREAK_DURATION             = 13.0                                   # lifetime of effect

C_FORWARDPROJECT                = 50.0          # how far forward to project the streak infront
								#  of the player

C_WARPSETSIZEMULSHIPRADIUS      = 3             # how tight the streaks are to the ship (size)
								#  multiplier to ship's radius.
...
class WarpTunnel:
	...
	def __buildtimer(self):
		...
		if not self.pTimerProcess:
			...
			self.pTimerProcess.SetDelay(self.__fUpdateTime * 0.3)
			self.pTimerProcess.SetPriority(App.TimeSliceProcess.NORMAL)            
	
	def SetWarpFactor(self, fWarpFactor):
		debug(__name__ + ", SetWarpFactor")
		if fWarpFactor == self.fWarpFactor:
			return
		fScaledWarpSpeed    = float(fWarpFactor*C_PULSEUPDATESCALEFACTOR)
		self.__fUpdateTime  = C_MAX_PULSEUPDATE - fScaledWarpSpeed
		self.fWarpFactor    = fWarpFactor
		if self.pTimerProcess != None:
			self.pTimerProcess.SetDelay(self.__fUpdateTime * 0.3)
	def updateValues(self):
		debug(__name__ + ", updateValues")
		fScaledWarpSpeed    = float(self.fWarpFactor*C_PULSEUPDATESCALEFACTOR)
		self.__fUpdateTime  = C_MAX_PULSEUPDATE - fScaledWarpSpeed
		if self.pTimerProcess != None:
			self.pTimerProcess.SetDelay(self.__fUpdateTime * 0.3)
	...
	def SparkStreak(self,pWarpingObject):
		...
		## Create a spark particle controller object.
		debug(__name__ + ", SparkStreak")
		pController = App.SparkParticleController_Create(2.0, MAX_STREAK_DURATION - self.fWarpFactor, 0.5) #dur, life, emitrate # 1.0, 6.0, 1.0
		pController.SetTailLength(GetRandomTailLength() * 0.18) # long tail
		SetupRandomColour(pController)

		## Setup lifetime alpha.
...
# update constants
def UpdateConstants():
	debug(__name__ + ", UpdateConstants")
	global lWarpingObjects, C_MAXWARP, C_MAX_PULSEUPDATE, C_PULSEUPDATESCALEFACTOR, C_MAXSTREAKSPERPULSE, MAX_STREAK_DURATION, C_FORWARDPROJECT, C_WARPSETSIZEMULSHIPRADIUS
	try:
		...
		C_MAX_PULSEUPDATE               = float(int(pConfigScript.Starstreaks.MaxPulseUpdate))
		C_PULSEUPDATESCALEFACTOR        = float(C_MAX_PULSEUPDATE / C_MAXWARP)
		C_MAXSTREAKSPERPULSE            = int(pConfigScript.Starstreaks.StreaksPerPulse)
		MAX_STREAK_DURATION             = float(int(pConfigScript.Starstreaks.StreakDuration))
		C_FORWARDPROJECT                = float(int(pConfigScript.Starstreaks.ForwardProject))
		C_WARPSETSIZEMULSHIPRADIUS      = int(pConfigScript.Starstreaks.SizeMultiplier)
		for pWarpTunnel in lWarpingObjects:
			pWarpTunnel.updateValues()
	except ImportError:
		print "Galaxy Charts LibStarstreaks couldn't load config file..."

def OverrideForwardProject(fValue):
	debug(__name__ + ", OverrideForwardProject")
	global lWarpingObjects, C_FORWARDPROJECT
	C_FORWARDPROJECT = C_FORWARDPROJECT + fValue
	for pWarpTunnel in lWarpingObjects:
		pWarpTunnel.updateValues()
