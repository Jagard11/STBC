...
def GetBasesOfSet(pSet):
	...
	if pRaceObj == None:
		return []
	lBases = []
	lBaseClasses = pRaceObj.GetBases()
	for pShip in pSet.GetClassObjectList(App.CT_SHIP):
...
# simple class to handle a random defence force wave, and possible reinforcements
...
class RandomDefenceForce:
	...
	def CreateShips(self, pMethodTimerEvent = None):
		...
		if pPlayer != None:
			...
			if IsShipFriendlyOfRace(pPlayer, self.Empire) == 1 or GetShipRace(pPlayer) == self.Empire:
...
def GetShipFundCost(oShip, bIncludeEscorts = 1):
	...
	import Galaxy
	try:
		print "GSFC: mark START"
		# start by getting the property set of the ship
		if type(oShip) == type(""):
			print "GSFC: mark check ship class - ", oShip
			# oShip is a string
			...
			print "GSFC: mark check ship - ", oShip.GetName(), "(", GetShipType(oShip), ")"
		...
		for i in range(pPropertyList.TGGetNumItems()):
			...
			if pShipProperty != None:
				print "GSFC: ____________________________________"
				print "GSFC: checking subsystem: ", pShipProperty.GetName().GetCString()
				...
				if App.HullProperty_Cast(pShipProperty) != None:
					...
					if pCloakSys.GetCloakStrength() <= 0.0:
						print "GSFC: cloak system sub cost, adding 100"
						fTotalFunds = fTotalFunds + 100.0
					else:
						print "GSFC: cloak system sub cost, adding", (pCloakSys.GetCloakStrength() * 10.0)
					...
					if App.ImpulseEngineProperty_Cast(pPoweredSys) != None:
						...
						iFormulaType = 3
						print "GSFC: impulse engine subsystem sub static cost, adding 200"
						fTotalFunds = fTotalFunds + 200.0
						fMaxAccel = pImpSys.GetMaxAccel()
						fMaxAngularAccel = pImpSys.GetMaxAngularAccel()
						fIEadd = (10.0 * fMaxAccel * fMaxAngularAccel * pImpSys.GetMaxAngularVelocity() * pImpSys.GetMaxSpeed())
						print "GSFC: impulse engine subsystem sub cost, adding", fIEadd
						fTotalFunds = fTotalFunds + fIEadd
					elif App.WarpEngineProperty_Cast(pPoweredSys) != None:
						iFormulaType = 3
						print "GSFC: warp engine subsystem sub static cost, adding 500"
						...
						fSFF = 4.0
						iFormulaType = 2
						print "GSFC: repair subsystem sub cost, adding", pRepairSys.GetMaxRepairPoints()*pRepairSys.GetNumRepairTeams()
						...
						fSFF = 4.0
						iFormulaType = 2
						fSSadd = ((pSensorSys.GetMaxProbes() * 45.0) + (pSensorSys.GetBaseSensorRange() / 20.0))
						print "GSFC: sensor subsystem sub cost, adding", fSSadd
						fTotalFunds = fTotalFunds + fSSadd
						...
						fS1add = ((FRONT_MaxShields + REAR_MaxShields + TOP_MaxShields + BOTTOM_MaxShields + LEFT_MaxShields + RIGHT_MaxShields) / 60.0)
						fTotalFunds = fTotalFunds + fS1add
						print "GSFC: shield subsystem hit points sub cost, adding", fS1add
						fS2add = (FRONT_ShieldCharge + REAR_ShieldCharge  + TOP_ShieldCharge + BOTTOM_ShieldCharge + LEFT_ShieldCharge + RIGHT_ShieldCharge)
						fTotalFunds = fTotalFunds + fS2add
						print "GSFC: shield subsystem recharge sub cost, adding", fS2add
						...
						if pWeaponSys.GetWeaponSystemType() == pWeaponSys.WST_TRACTOR:
							print "GSFC: tractor beam system subsystem static sub cost, adding 300"
							fTotalFunds = fTotalFunds + 300.0
						elif pWeaponSys.GetWeaponSystemType() == pWeaponSys.WST_TORPEDO:
							print "GSFC: torpedo system subsystem static sub cost, adding 120"
							...
							if pTorpSys != None:
								for index in range(pTorpSys.GetNumAmmoTypes()):
									...
									print "GSFC: torpedo system subsystem sub cost, checking AMMO:", sTorpScript, "(cost:", "|ammo count", iTorpAmmo, ")"
									fTotalFunds = fTotalFunds + (torpcost * iTorpAmmo)
									print "GSFC: torpedo system subsystem sub cost, adding (for ammo): ", (torpcost * iTorpAmmo)
						elif pWeaponSys.GetWeaponSystemType() == pWeaponSys.WST_PULSE:
							print "GSFC: pulse system subsystem static sub cost, adding 200"
							fTotalFunds = fTotalFunds + 200.0
						elif pWeaponSys.GetWeaponSystemType() == pWeaponSys.WST_PHASER:
							print "GSFC: phaser beam system subsystem static sub cost, adding 250"
					...
					if App.TorpedoTubeProperty_Cast(pWeapon) != None:
						...
						print "GSFC: torpedo tube subsystem sub cost, adding", (torpsfiredpersecond * 200.0), " ||TorpsPerSec:", torpsfiredpersecond
					elif App.TractorBeamProperty_Cast(pWeapon) != None:
						pTractor = App.TractorBeamProperty_Cast(pWeapon)
						fSFF = 4.0
						print "GSFC: tractor projector subsystem sub cost, adding", ((pTractor.GetMaxDamage() + pTractor.GetMaxDamageDistance()) / 2.0)
						...
						try:
							...
							print "GSFC: pulse cannon subsystem sub cost, checking pulse:", pPulse.__name__, " (cost:", pulsecost, ")"
						except:
						...
						if secsforoneshot != 0.0:
							...
							shotspersec = 0.01
						print "GSFC: pulse cannon subsystem sub cost, adding", (pulsecost * shotspersec)
						...
						print "GSFC: beam array subsystem sub cost, adding", beamcost
						fTotalFunds = fTotalFunds + beamcost
				if iFormulaType == 1:
					fFT1add = ((fMaxHP + ((bIsCritical-1) * -0.8 * fMaxHP) + (fRepairComp * (fMaxHP/50.0)))/fSFF)
					print "GSFC: adding cost formula type 1: ", fFT1add
					fTotalFunds = fTotalFunds + fFT1add
				elif iFormulaType == 2:
					fFT2add = ((fMaxHP + (bIsCritical* -0.8 * fMaxHP) + (fRepairComp * (fMaxHP/75.0)))/fSFF)
					print "GSFC: adding cost formula type 2: ", fFT2add
					fTotalFunds = fTotalFunds + fFT2add
				elif iFormulaType == 3:
					print "GSFC: not adding cost - formula type 3"
					pass
		...
		if pPowerProp != None:
			...
			bIsCritical = pShipProperty.IsCritical()
			fRepairComp = pShipProperty.GetRepairComplexity()
			fPG1add = ((fMaxHP + ((bIsCritical-1) * -0.8 * fMaxHP) + (fRepairComp * (fMaxHP/50.0)))/2.0)
			print "GSFC: additional power generator formula type 1 sub cost, adding", fPG1add
			fPG2add = ((pPowerProp.GetMainConduitCapacity() - pPowerProp.GetPowerOutput()) + pPowerProp.GetBackupConduitCapacity())
			print "GSFC: additional power generator output and battery relation sub cost, adding", fPG2add
			fPG3add = ((pPowerProp.GetPowerOutput() - fPowerUsed)*2)
			print "GSFC: additional power generator output and consumption relation sub cost, adding", fPG3add
			fTotalFunds = fTotalFunds + fPG1add
			fTotalFunds = fTotalFunds + fPG2add
			fTotalFunds = fTotalFunds + fPG3add
			if fCloakPower > 0.0:
				fPG4add = ((pPowerProp.GetBackupBatteryLimit() / fCloakPower) * 10.0)
				print "GSFC: additional power generator battery and cloak relation sub cost, adding", fPG4add
				fTotalFunds = fTotalFunds + fPG4add
		...
		for i in range(pOEPList.TGGetNumItems()):
			...
			if pOEP != None:
				if pOEP.GetEmittedObjectType() == pOEP.OEP_SHUTTLE:
					print "GSFC: can launch shuttles, adding 1000"
					...
					print "GSFC: can launch probes, adding 100"
					fTotalFunds = fTotalFunds + 100.0		
		...
		if hasattr(pShipDef, "dTechs") == 1:
			for sTechName in pShipDef.dTechs.keys():
				if sTechName == "Ablative Armour":
					...
					print "GSFC: FTECH - ablative armor, adding", fAblativeCost
					...
					if oRegShields != 0.0:
						...
						print "GSFC: FTECH - regenerative shields, adding", (7000.0 / oRegShields)
					...
					if oMultShields != 0.0:
						...
						print "GSFC: FTECH - multivectral shields, adding", (6000.0 / (100.0 / oMultShields))
					...
					if oReflecShields != 0.0:
						...
						print "GSFC: FTECH - reflector shields, adding", (10000.0 / (100.0 / oReflecShields))
				elif sTechName == "Phase Cloak":
					oPhaseCloak = pShipDef.dTechs[sTechName]
					print "GSFC: FTECH - phase cloak, adding 10000.0"
					fTotalFunds = fTotalFunds + 10000.0
				elif sTechName == "Breen Drainer Immune":
					oBDI = pShipDef.dTechs[sTechName]
					print "GSFC: FTECH - breen drainer immune, adding 3000.0"
					...
					print "GSFC: FTECH - FED ablative armor, adding", (2500.0 * len(lPlateList))
					...
					for sSubsystem in oAdvHull.keys():
						...
						print "GSFC: FTECH - AdvancedHull system, adding", (2000.0 - (20.0 * fChance)) + (2000.0 - (20.0 * fDmgPerc))
					...
					for sWeaponType in oAutoTarget.keys():	
						if sWeaponType in ["Pulse", "Phaser", "Torpedo"]:
							...
							print "GSFC: FTECH - AutoTargeting system, adding", (100.0 * (iMaxSecTargets + iMaxSysTargets))
				else:
					print "GSFC: FTECH - regular tech, adding 5000.0"
			...
			if pRaceObj != None:
				...
				if lEscorts != None:
					for sEscort in lEscorts:
						...
						fEcost = GetShipFundCost(sEscort, 0)
						fTotalFunds = fTotalFunds + fEcost
						print "GSFC: adding escort "+sEscort+" cost, adding", fEcost
		...
		if pShipDef != None and pGodRace != None:
			if pShipDef.shipFile in pGodRace.myShips:
				...
				print "GSFC: classified as GOD ship, adding 150000.0"
		...
		if hasattr(pShipDef, "fShipCostModifier"):
			...
			if (fTotalFunds + fValueToAdd) >= 100.0:
				...
				print "GSFC: adding Ship Cost Modifier, adding", fValueToAdd

		# and finally return the cost.
		print "GSFC: mark final. Final Cost:", fTotalFunds
		return fTotalFunds
	except:
