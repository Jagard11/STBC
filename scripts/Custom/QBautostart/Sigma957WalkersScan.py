#**********************************************************************************************************************
#	Sigma957WalkersScan.py, based from SG1EMPulse.py
#**********************************************************************************************************************
#       I did minimal changes meant to disable only the main power system
...
#**********************************************************************************************************************
EMPMaxRadius = 180 		# Have to figure out the distance divide by 6 to get km displayed on screen 90 ~ 15km, 120 ~ 20km, 180  ~ 30km
#**********************************************************************************************************************
...
MODINFO = {     "Author": "wowsher",
				"Version": "0.33",
				"License": "GPL",
				"Description": "EM Pulse knocks out ships in a ~30km radius around your ship",
				"needBridge": 0
			}

def init():
	#if not Libs.LibEngineering.CheckActiveMutator("Sigma Walkers Scan"):
	#	return
...
def BuildMenu():
	global pMain, pChargeButton, pReleaseButton, thiswasdone
	# define buttons on Science menu for controlling the EMP
	pScienceMenu = Bridge.BridgeUtils.GetBridgeMenu("Science")

	if (pScienceMenu != None):
                if (thiswasdone == '0'):
                        App.g_kEventManager.AddBroadcastPythonFuncHandler(App.ET_SCAN,		pScienceMenu,	__name__ + ".ReleaseEMPulse")
                        thiswasdone = '1'
	#	pPulseMenu=App.STMenu_CreateW(App.TGString("Molecular Scan"))
	#	if (pPulseMenu != None):
	#		pEngMenu.PrependChild(pPulseMenu)
	#		pReleaseButton = Libs.LibEngineering.CreateMenuButton("Release", "Science", __name__ + ".ReleaseEMPulse",0,pPulseMenu)
        #                pReleaseButton.SetEnabled()
	#		#pChargeButton = Libs.LibEngineering.CreateMenuButton("Enable", "Science", __name__ + ".EnableEMWeapon",0,pPulseMenu)
	#		#pReleaseButton.SetDisabled()
        #                pEngMenu.AddChild(BridgeUtils.CreateBridgeMenuButton("Initiate",		App.ET_SCAN,	App.CharacterClass.EST_SCAN_AREA,	pScienceMenu))
	## BuildMenu

def RemoveMenu():
	pEngMenu = Bridge.BridgeUtils.GetBridgeMenu("Science")
	if (pEngMenu != None):
		pPulseMenu = pEngMenu.GetSubmenu("Molecular Scan")
		if (pPulseMenu != None):
...
def ShipCheck(pObject, pEvent):
	...
	if (GetShipType(pPlayer) == "SigmaWalkerScienceLab"):
                imasigma = 1
		BuildMenu()
        else:
                imasigma = 0
		...
		for pObject in lObjects:
			if pObject.IsTypeOf(App.CT_SHIP):					# Only check further if the object is a ship
				if DistanceToTarget(pObject) < EMPMaxRadius:	# now check proximity to target, if within range then shut it down for a certain time
					...
					#print sShipName
					lSubs = [pShip.GetPowerSubsystem(), pShip.GetCloakingSubsystem()]
...
GFX = 'scripts/Custom/Babylon5/effects/'
...
def InitiateWaveSeq(pObject, pEvent):
	...
	pSequence.Play()
	pSound = App.TGSound_Create("scripts/Custom/Babylon5/effects/systems_shut_down.mp3", "EMEnabled", 0)
	pSound.Play()
...
def InitiateWaveSeq2(pObject, pEvent):
	...
	pSequence = App.TGSequence_Create ()
	pSound = App.TGSound_Create("scripts/Custom/Babylon5/effects/em_pulse_short.mp3", "EMPulse", 0)
	pSound.Play()
...
def CreateGFX():
	...
	fSize     = 120 #25  appears to be starting size of effect
	pSequence = App.TGSequence_Create()   
