...
def CreateEntries(pObject, pEvent):
        ...
        pIconWindow = App.STStylizedWindow_CreateW("StylizedWindow", "NoMinimize", App.TGString("Mission: Retake DS9"), 0.0, 0.0, None, 1, 0.30, 0.30, kColor)
        ...
        pText = App.TGParagraph_CreateW(App.TGString("Captain, it's time now to return to DS9. We have received word that the Dominion is about to disarm the cloaked minefield we have placed. If this happens the Dominion will be able to receive reinforcements from the Gamma Quadrant. We can't let this happen. You and your ship, the USS Trinculo, are ordered to join the task force. We do no expect the Klingons to join us so you will be on your own. Godspeed Captain."), pBriefingWindow.GetMaximumInteriorWidth(), None, '', pBriefingWindow.GetMaximumInteriorWidth(), App.TGParagraph.TGPF_WORD_WRAP | App.TGParagraph.TGPF_READ_ONLY)
...
def TextSequence(pPane):
        ...
        pAction = LineAction("Mission: Retake DS9", pPane, 3, 6, 16)
...
def KlingonArrival():
        ...
        pMission = App.Game_GetCurrentGame().GetCurrentEpisode().GetCurrentMission()
        DS9FXLifeSupportLib.ClearFromGroup("IVK Ykir")
        ...
        pMission = App.Game_GetCurrentGame().GetCurrentEpisode().GetCurrentMission()
        DS9FXLifeSupportLib.ClearFromGroup("IKV Nawrya")
