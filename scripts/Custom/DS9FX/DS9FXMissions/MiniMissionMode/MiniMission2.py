...
pScanPTS = 0
pPaneID = App.NULL_ID
pName = MissionIDs.MM2
sName = "Anomalous Readings"
sObjectives = "-Get in close range to scan the comet\n-Scan the comet for 1 minute\n-See what happens next"
sBriefing = "We are getting some strange readings from the Comet Alpha. You are required to scan the comet and check out the readings. Who knows, you might even discover something new Captain..."

def Briefing():
        global pMainPane, pPane

        reload (DS9FXSavedConfig)
        if not DS9FXSavedConfig.CometSelection == 1:
                CometNotSelected()
                return 0
...
def CreateEntries(pObject, pEvent):
        ...
        pIconWindow = App.STStylizedWindow_CreateW("StylizedWindow", "NoMinimize", App.TGString("Mission: Anomalous Readings"), 0.0, 0.0, None, 1, 0.30, 0.30, kColor)
...
def SpawnCardies():
        ...
        loadspacehelper.CreateShip(DS9FXShips.Galor, pSet, "Galor 1", "FriendlyPos1")

        pMission = App.Game_GetCurrentGame().GetCurrentEpisode().GetCurrentMission()
        DS9FXLifeSupportLib.ClearFromGroup("Galor 1")
        ...
        loadspacehelper.CreateShip(DS9FXShips.Galor, pSet, "Galor 2", "FriendlyPos2")

        pMission = App.Game_GetCurrentGame().GetCurrentEpisode().GetCurrentMission()
        DS9FXLifeSupportLib.ClearFromGroup("Galor 2")
        ...
        loadspacehelper.CreateShip(DS9FXShips.Galor, pSet, "Galor 3", "FriendlyPos3")

        pMission = App.Game_GetCurrentGame().GetCurrentEpisode().GetCurrentMission()
        DS9FXLifeSupportLib.ClearFromGroup("Galor 3")
        ...
        loadspacehelper.CreateShip(DS9FXShips.Galor, pSet, "Galor 4", "FriendlyPos4")

        pMission = App.Game_GetCurrentGame().GetCurrentEpisode().GetCurrentMission()
        DS9FXLifeSupportLib.ClearFromGroup("Galor 4")
        ...
        loadspacehelper.CreateShip(DS9FXShips.Galor, pSet, "Galor 5", "FriendlyPos5")

        pMission = App.Game_GetCurrentGame().GetCurrentEpisode().GetCurrentMission()
        DS9FXLifeSupportLib.ClearFromGroup("Galor 5")
...
def CometNotSelected():
        ...
        pSequence = App.TGSequence_Create()
        pSequence.SetUseRealTime (1)
        pSequence.AppendAction(CometNotSelectedSeq(pPane))
        pPaneID = pPane.GetObjID()
        pSequence.Play()

def CometNotSelectedSeq(pPane):
        pSequence = App.TGSequence_Create()
        pSequence.SetUseRealTime (1)

        pAction = LineAction("Comet Alpha is not enabled, Mission is unplayable...", pPane, 3, 6, 12)
