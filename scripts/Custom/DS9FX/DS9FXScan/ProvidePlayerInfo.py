...
def ScanPlayer():
    ...
    pShip = DS9FXLifeSupportLib.GetShipType(pPlayer)
    if not pShip:
        return 0
    ...
    if Foundation.shipList.has_key(pShip):
        ...
        if fShip:
            ...
            if hasattr(fShip, "fCrew"):
                ...
                iMaxCrew = DS9FXLifeSupportLib.CalculateMaxCrew(pPlayer)
                bNoCrew = 0
            if bNoCrew:
                iCurrCrew = "N/A"
                iMaxAllowedCrew = "N/A"
            ...
            if IsNumeric(iMaxAllowedCrew) and IsNumeric(iCurrCrew):
                if iCurrCrew >= iMaxAllowedCrew:
                    ...
                    if pFriendly:
                        if pFriendly.IsNameInGroup(pPlayer.GetName()):
                            sCanTransportCrew = "Yes"
                        else:
                            sCanTransportCrew = "No"
                    else:
                        sCanTransportCrew = "No"
            if CaptureShip.ShieldCheck(pPlayer.GetObjID()) and not bNoCrew:
                sCanBeBoarded = "Yes"
            else:
                if not bNoCrew:
                    sCanBeBoarded = "No"
            if IsNumeric(iCurrCrew):
                if iCurrCrew <= 0:
                    ...
                    for pSide in lSides:
                        if pSide:
                            if pSide.IsNameInGroup(pPlayer.GetName()):
                                bCanRecover = 0
                                break
                    if bCanRecover:
                        sCanBeRecovered = "Yes"
