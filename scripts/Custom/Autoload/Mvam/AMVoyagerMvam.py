...
MvamShips = ["AMVoyager", "ArmoredVoyager"]
...
IconNames = ["Activate ARMOUR", "Deactivate$Rein"]
...
MvamDistances[0].SetXYZ(-0.020155, -0.082234, 0.03128)
...
MvamReinDistances[0].SetXYZ(0.0, -2.762963, -1.3090572)
...
MvamAiName = "Custom.Sneaker.Mvam.AMVoyagerAI"
...
def ArmoredVoyager(pObject, pEvent):
...
###TO CHANGE: change the last word in quotes in the next line to the name of the def. (ie: "ArmoredVoyager")
		Custom.Sneaker.Mvam.Seperation.Seperation(snkMvamModule, "ArmoredVoyager")
...
def CheckSeperate (pShip):
	#this means if the ship's hull is at or below 75%, we seperate
	if (pShip.GetHull().GetConditionPercentage() <= 0.85):
		return 1
	else:
            return 0 
