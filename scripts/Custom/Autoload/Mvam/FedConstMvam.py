...
MvamShips = ["FedConst", "FedConstOpen"]
...
IconNames = ["Open Dock", "Close Dock$Rein"]
...
MvamDistances[0].SetXYZ(0, 1.356867, 1.489279)
...
MvamReinDistances[0].SetXYZ(0, 1.356867, 1.488579)
...
############# file too, if you decide to have one. Leave blank ("") if you don't want a sound
BridgeReinSoundName = "OutriggerPowerdown"
...
def FedConstOpen(pObject, pEvent):
...
###TO CHANGE: change the last word in quotes in the next line to the name of the def
		Custom.Sneaker.Mvam.Seperation.Seperation(snkMvamModule, "FedConstOpen")
...
def CheckSeperate (pShip):
	#this means if the ship's hull is at or below 25%, we seperate
	if (pShip.GetHull().GetConditionPercentage() <= 0.75):
		return 1
	else:
		return 0
