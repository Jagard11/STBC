...
MvamShips = ["EAStealthStarfury", "EAStealthStarfuryEscapePod", "EAStealthStarfuryNoEscapePod"]
...
IconNames = ["Launch cockpit", "Reintegrate$Rein"]
...
MvamDistances[1].SetXYZ(0.0, 0.000000, 0.000000)
...
def EAStealthStarfuryEscapePod(pObject, pEvent):
...
###TO CHANGE: change the last word in quotes in the next line to the name of the def
		Custom.Sneaker.Mvam.Seperation.Seperation(snkMvamModule, "EAStealthStarfuryEscapePod")
...
def EAStealthStarfuryNoEscapePod(pObject, pEvent):
...
###TO CHANGE: change the last word in quotes in the next line to the name of the def
		Custom.Sneaker.Mvam.Seperation.Seperation(snkMvamModule, "EAStealthStarfuryNoEscapePod")
...
def CheckSeperate (pShip):
	...
	if (pEnemies.IsNameInGroup(pShip.GetName())):
		#seperate if 90% damaged
		if ((pShip.GetHull().GetConditionPercentage() <= 0.1) and (snkAiRanSep == 4)):
			return 1
