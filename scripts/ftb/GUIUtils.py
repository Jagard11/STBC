...
def GetTacticalMenu():
    debug(__name__ + ", GetTacticalMenu")
    pTactCtrlWindow = App.TacticalControlWindow_GetTacticalControlWindow()
    pDatabase = App.g_kLocalizationManager.Load("data/TGL/Bridge Menus.tgl")
    if(pDatabase is None):
        return 0
    return pTactCtrlWindow.FindMenu(pDatabase.GetString("Tactical"))


def FindButtonPosInMenu( menu, button):
    debug(__name__ + ", FindButtonPosInMenu")
    retval = 0
...
def GetNumChildrenThatCastTo( pane, cast):
    debug(__name__ + ", GetNumChildrenThatCastTo")
    retval = 0
...
def CreateIntButton( name, eventType, eventDest, intVal, tglDatabase = None):
    debug(__name__ + ", CreateIntButton")
    debug(__name__ + ", CreateIntButton Event: " + str(eventType))
    if not eventType:
        return
    button = None
